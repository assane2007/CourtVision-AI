...
export async function POST(req: NextRequest) {
  try {
    ...
    const { imageBase64, drillName, category, drillInstructions } = body

    if (!imageBase64 || !drillName || !category) {
      return NextResponse.json({ error: 'Image, nom et catégorie requis' }, { status: 400 })
    }
...
Sois précis dans tes retours. Si la forme est excellente (score > 85), félicite. Si des corrections sont nécessaires, sois spécifique sur ce qui doit changer.`
    ...
    try {
      ...
      result = jsonMatch ? JSON.parse(jsonMatch[0]) : { score: 50, feedback: content, issues: [], goodPoints: [] }
    } catch {
    ...
    // Ensure feedback is in French and concise
